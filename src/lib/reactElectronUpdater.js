...
import { GitHubReleaseUtil } from './util/github-release.util';
import { ElectronAppUtil } from './util/electron-app.util';
import { FsCommonUtil } from './util/fs-common.util';
import { AppUpadateStatus } from './util/model/app-update-status.model';
import { ElectronInstallerUtil } from './util/electron-installer.util';
import { DownloadNotifierType, DownloadStatus } from './util/model/download-status.model';
      ...
      this._gitHubReleaseUtil.hasReleaseInfo(url, releaseInfo).subscribe(hasReleaseInfo=>{
          if(hasReleaseInfo==="available"){
              this._gitHubReleaseUtil.getLatestReleaseInfo(url, releaseInfo).subscribe(appReleaseInfo=>{
                  ...
                  if(this.gt(appReleaseInfo.version,appUpadateStatus.currentAppVersion)){
    ...
    render() {
      return (
        <ChildComponent
            ...
            downloadAndInstall={()=>this.downloadAndInstall()}
